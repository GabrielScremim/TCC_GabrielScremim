...
ResultadoBenchmark executar_benchmark(int m, int n, int num_repeticoes) {
    ...
    for (int i = 0; i < num_repeticoes; i++) {
        ...
        size_t mem_antes = memoria_usada_MB();
        ...
        exec->n = n;
        exec->execucao = i + 1;
        exec->tempo_total = (double)(fim_total - inicio_total) / CLOCKS_PER_SEC;
        exec->tempo_construcao = (double)(fim_construcao - inicio_construcao) / CLOCKS_PER_SEC;
        exec->tempo_simplex = (double)(fim_simplex - inicio_simplex) / CLOCKS_PER_SEC;
        exec->memoria_mb = (double)(mem_depois - mem_antes);
...
int main() {
    ...
    sprintf(nome_json, "benchmark_c_%s.json", timestamp);
    sprintf(nome_csv, "benchmark_c_%s.csv", timestamp);
